...
namespace nexuspool
{
	...
	bool Pool::init(std::string const& pool_config_file)
	{
		...
		m_pool_manager = std::make_shared<Pool_manager>(m_io_context, m_config, m_network_component->get_socket_factory(), m_persistance_component->get_data_writer_factory());
